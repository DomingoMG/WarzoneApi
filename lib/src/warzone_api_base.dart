...
class WarzoneApi {
  ...
  Future<WarzoneProfile> fetchUser(
    ...
    String url =
        '${Global.trackerApi}/profile/${platform.name.toLowerCase()}/$username'
            .replaceAll('#', '%23');
  ...
  Future<List<WarzoneSearch>> search(
      {required String username, Platform platform = Platform.atvi}) async {
    String url =
        '${Global.trackerApi}/search?platform=${platform.name.toLowerCase()}&query=$username'
            .replaceAll('#', '%23');
    ...
    if (response.statusCode == 200) {
      ...
      return (dataJson['data'] as List)
          .map((users) => WarzoneSearch.fromMap(users))
          .toList();
    } else {
      // Users not found
      return [];
  ...
  Future<WarzoneMatch> fetchMatchFromUser(
